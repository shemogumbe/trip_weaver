...
class GooglePlacesClient:
    ...
    Inputs:
        - hobby: a descriptive activity category (e.g., "fine dining", "golf")
        - location: city or destination name

    Output:
        - list[dict]: Google Places results with keys like name, vicinity, rating, place_id

    Error modes:
        - Raises IntegrationError if API key is not configured
        - Returns [] on upstream transient errors; logs warnings
    """

    def __init__(self) -> None:
        if not GOOGLE_PLACES_API_KEY:
            # Fail fast to prevent silent fallback to hallucinations
            raise IntegrationError("Google Places API key not configured. Set GOOGLE_PLACES_API_KEY in env.")
        self.client = gmaps
    ...
    def get_location_coordinates(self, location: str) -> Optional[Tuple[float, float]]:
        ...
        if not self.client:
            raise IntegrationError("Google Places client not initialized")
...
# Global instance (lazy / tolerant): only create when key is present
google_places_client = GooglePlacesClient() if GOOGLE_PLACES_API_KEY else None